...
function attitude_widgets_init() {
	...
	register_sidebar( array(
        ...
        // Registering second right sidebar
	register_sidebar( array(
		'name' 				=> __( 'Social Sidebar', 'attitude' ),
		'id' 					=> 'social_sidebar',
		'description'   	=> __( 'Widgets in der rechten Sidebar ganz unten', 'attitude' ),
		'before_widget' 	=> '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  	=> '</aside>',
		'before_title'  	=> '<menu>',
		'after_title'   	=> '</menu>'
...
<!-- geändert von class="more-link" zu class="readmore" -->
						<a class="readmore" title="<?php the_title_attribute(); ?>" href="<?php the_permalink(); ?>"><?php _e( 'Read more','attitude' ); ?></a>
					</div><!-- .one-third --> 					
				<?php endwhile;
		 		// Reset Post Data
	 			wp_reset_query(); 
				...
				<?php 
	 			while( $get_featured_pages->have_posts() ):$get_featured_pages->the_post();
					...
					<div class="one-fourth">
						...
						?>
						<h3 class="custom-gallery-title"><a href="<?php the_permalink(); ?>" title="Lies jetzt: <?php echo $page_title; ?>"><?php echo $page_title; ?></a></h3>
					</div><!-- .one-fourth -->			
				<?php endwhile;
		 		// Reset Post Data
	 			wp_reset_query(); 
